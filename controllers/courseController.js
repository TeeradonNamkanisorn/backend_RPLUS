...
exports.validateCourseParams = async (req, res, next) => {
  try {
    ...
    if (!validLevels.includes(req.body.level))
      createError("Invalid course level");
...
exports.createCourse = async (req, res, next) => {
  ...
  try {
    ...
    const result = await Course.create({
      ...
      videoPublicId: req.imageData.public_id,
    });
  ...
  try {
    ...
    const course = await Course.findOne({
      ...
      order: [
        ...
        [Chapter, Lesson, "lessonIndex", "ASC"],
      ],
    });
...
exports.updateCourse = async (req, res, next) => {
  try {
    ...
    if (course.teacherId !== userId)
      createError("you are not authorized to edit", 403);
    ...
    if (course.videoLink && req.videoData?.secure_url) {
      const result = await destroy(course.videoPublicId, {
        resource_type: "video",
      });
...
exports.changePrice = async (req, res, next) => {
  try {
    ...
    const { courseId } = req.params;
    console.log("change price", price, courseId);
...
module.exports.getStudentOwnedCourses = async (req, res, next) => {
  try {
    ...
    const numberCompletedObj = numberCompleted.reduce((acc, cur) => {
      acc[cur.courseId] = cur.completedLessons;
      return acc;
    }, {});
    const numberLessonObj = numberLesson.reduce((acc, cur) => {
      acc[cur.courseId] = cur.totalLessons;
      return acc;
    }, {});
...
module.exports.getStudentOwnedCourseById = async (req, res, next) => {
  try {
    ...
    let completedLessons = await StudentLesson.findAll({
      where: {
        studentId,
        courseId,
        status: "COMPLETED",
    ...
    let previouslyCompletedLessons = await StudentLesson.findAll({
      where: {
        studentId,
        courseId,
    ...
    previouslyCompletedLessons = JSON.parse(
      JSON.stringify(previouslyCompletedLessons)
    );
    previouslyCompletedLessons = previouslyCompletedLessons.map(
      (el) => el.lessonId
    );
